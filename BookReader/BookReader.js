...
function BookReader() {
    ...
    this.logoURL = 'http://library.stanford.edu';
...
BookReader.prototype.drawLeafsOnePage = function() {
    ...
    for (i=0; i<this.numLeafs; i++) {
				var height = parseInt(this.getDimensionForLevel(this.getPageHeight(i), this.getMaxJP2Level(i), this.reduce), 10);
        leafBottom += height;
    ...
    if (firstIndexToDraw != this.firstIndex) {
		...
		//console.log("firstIndexToDraw, firstIndex = " + firstIndexToDraw + ',' + this.firstIndex); //jc
    ...
    for (i=0; i<firstIndexToDraw; i++) {
				leafTop += parseInt(this.getDimensionForLevel(this._getPageHeight(i), this.getMaxJP2Level(i), this.reduce), 10) + 10;
    }
    ...
    for (i=0; i<indicesToDisplay.length; i++) {
        var index = indicesToDisplay[i];    
				var height = parseInt(this.getDimensionForLevel(this._getPageHeight(index), this.getMaxJP2Level(index), this.reduce), 10);

        if (BookReader.util.notInArray(indicesToDisplay[i], this.displayedIndices)) {            
				    var width = parseInt(this.getDimensionForLevel(this._getPageWidth(index), this.getMaxJP2Level(index), this.reduce), 10);
    ...
    for (i=0; i<rowsToDisplay.length; i++) {
        if (BookReader.util.notInArray(rowsToDisplay[i], this.displayedRows)) {    
            ...
            for (j=0; j<leafMap[row].leafs.length; j++) {
                ...
                //var thumbReduce = Math.floor(this.getPageWidth(leaf) / this.thumbWidth); //jc
								thumbReduce = this.reduce;
...
BookReader.prototype.jumpToIndex = function(index, pageX, pageY) {
    ...
    if (this.constMode2up == this.mode) {
        ...
        var leafTop = this.onePageGetPageTop(index);
				//console.log("jumpToIndex = " + index); //jc
			  //console.log("firstIndex = " + this.firstIndex); //jc
        this.firstIndex=index;
...
BookReader.prototype.prepareThumbnailView = function() {
    ...
    //this.reduce = this.getPageWidth(0)/this.thumbWidth;
		this.reduce = this.getLevelForDimension(this.thumbWidth, this.getPageWidth(0), this.getMaxJP2Level(0)) + 1;
...
BookReader.prototype.onePageGetAutofitWidth = function() {
    ...
    //return (this.getMedianPageSize().width + 0.0) / ($('#BRcontainer').attr('clientWidth') - widthPadding * 2);
		//console.log("onePageGetAutofitWidth = " + this.getLevelForDimension(($('#BRcontainer').attr('clientWidth') - widthPadding * 2), (this.getMedianPageSize().width + 0.0), 4)); //jc
		return(this.getLevelForDimension(($('#BRcontainer').attr('clientWidth') - widthPadding * 2), (this.getMedianPageSize().width + 0.0), this.getMaxJP2Level(0)));
}
...
BookReader.prototype.onePageGetAutofitHeight = function() {
    //return (this.getMedianPageSize().height + 0.0) / ($('#BRcontainer').attr('clientHeight') - this.padding * 2); // make sure a little of adjacent pages show
		//console.log("onePageGetAutofitHeight = " + this.getLevelForDimension(($('#BRcontainer').attr('clientHeight') - this.padding * 2), (this.getMedianPageSize().height + 0.0), 4)); //jc
		return(this.getLevelForDimension(($('#BRcontainer').attr('clientHeight') - this.padding * 2), (this.getMedianPageSize().height + 0.0), this.getMaxJP2Level(0)));
}
...
BookReader.prototype.next = function() {
						//console.log("next(): firstIndex = " + this.firstIndex); //jc
						//console.log("next(): lastDisplayableIndex = " + this.lastDisplayableIndex()); //jc
    if (2 == this.mode) {
        this.autoStop();
...
BookReader.prototype._getPageURI = function(index, reduce, rotate) {
    ...
    if ('undefined' == typeof(reduce)) {
		...
		if ('undefined' == typeof(reduce)) {
		  reduce = this.getLevelForDimension(this.twoPage.height, this.getPageHeight(index), this.getMaxJP2Level(index));
		}

		if (this.mode == this.constMode2up) {
	    reduce = this.validateJP2Level(reduce+1, this.getMaxJP2Level(index));					 
		}
...
BookReader.prototype.ttsScrollToChunk = function(chunk) {
    ...
    for (i=0; i<this.ttsIndex; i++) {
        h = parseInt(this.Height(i)/this.reduce); 
        leafTop += h + this.padding;
    }
...
// Moved from BookReaderJSSimple.js
// -----------------------------------------------
...
BookReader.prototype.getThumbnailHeight = function(pageWidth, pageHeight) {
  var djatokaMinLevelLengthCutoff = 92;
  var lengthLongestSide = Math.max(parseInt(pageWidth), parseInt(pageHeight));
  var thumbnailHeight = lengthLongestSide;

  while (thumbnailHeight > djatokaMinLevelLengthCutoff) {
    thumbnailHeight = parseInt(thumbnailHeight/2, 10);
  }

  if (parseInt(pageHeight, 10) < parseInt(pageWidth, 10)) {
    thumbnailHeight = parseInt((pageHeight/pageWidth)*thumbnailHeight, 10);   
  }

  return thumbnailHeight;
}

BookReader.prototype.getDimensionForLevel = function(maxDimension, maxJP2Level, currentLevel) {
  var noTimesToHalve = parseInt(maxJP2Level - currentLevel, 10);
  var currentDimension = maxDimension;
  
  if (noTimesToHalve > 0) {
    for (i = 1; i <= noTimesToHalve; i++) {
      currentDimension = parseInt(currentDimension >> 1);
    }
  }
...
BookReader.prototype.getLevelForDimension = function(containerDimension, maxDimension, maxJP2Level) {
  for (var level = maxJP2Level; level >= 0; level--) {
    if (maxDimension <= containerDimension) {
      return level;
    }
    maxDimension = parseInt(maxDimension >> 1);
  }
}

BookReader.prototype.validateJP2Level = function(level, maxJP2Level) {
  if (level < 0) { level = 0; }
  if (level > maxJP2Level) { level = maxJP2Level; }
  return level;
}
